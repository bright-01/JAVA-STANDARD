...
public class ArrayListClass {
     ...
     *  ==> 크기 변경 X, 크기를 변경하기 위해서는 새로운 배열을 생성 후 복사해야함. --> 배열의 크기를 크게 잡으면 메모리 낭비
     *  ==> 비순차적 데이터 추가 삭제에 시간이 많이 걸린다. 맨끝 앞은 괜찮음
     *  불연속으로 존재하는 데이터 연결 각 주소값이 어디있는지 모름 다 다름. ==> 노드 ( 다음 노드의 주소를 가지고 있다 )
     * =====> 데이터삭제 노드의 참조만 변경하면 된다. 추가도 마찬가지로 참조만 변경하면 쉽게 가능
     * 더블리 링크드 리스트 - 앞뒤 참조를 다 가지고 있음
     * 더블리 써큘러 링크드 리스트 - 맨앞이랑 뒤랑 연결  ( 채널이랑 같.. )
     *
     *
